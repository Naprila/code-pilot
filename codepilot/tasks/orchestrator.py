...
from codepilot.tasks.task_store import SQLiteTaskStore
from codepilot.tasks.executor import run_subtask_agent
from codepilot.tasks.planner import create_plan
from codepilot.tasks.approval import present_plan_for_approval
from codepilot.tasks.recovery import RecoveryManager
from codepilot.context.indexers.factory import get_indexer
...
async def handle_plan_command(goal: str) -> None:
    ...
    """
    db_path = config.get("tasks", {}).get("db_path", ".codepilot/tasks.db")
    store = SQLiteTaskStore(db_path)
    recover = RecoveryManager(store)

    project_id = store.get_latest_resumable_project()
    if project_id:
